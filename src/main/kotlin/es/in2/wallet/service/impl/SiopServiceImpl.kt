...
class SiopServiceImpl(
     ...
     */
    override fun getSiopAuthenticationRequest(
        userUUID: UUID,
        siopAuthenticationRequestUri: String
    ): MutableList<String> {
        ...
        return processSiopAuthenticationRequest(userUUID, siopAuthenticationRequest)
    }
     ...
     */
    override fun processSiopAuthenticationRequest(
        userUUID: UUID,
        siopAuthenticationRequest: String
    ): MutableList<String> {
        ...
        val verifiableCredentialList =
            personalDataSpaceService.getVerifiableCredentialsByVcType(userUUID, scopeList)
    ...
    private fun getSiopAuthenticationRequestInJwsFormat(siopAuthenticationRequestUri: String): String {
        val client = HttpClient.newBuilder().build()
        val request = HttpRequest.newBuilder()
            .uri(URI.create(siopAuthenticationRequestUri))
            .GET()
            .build()
        val response = client.sendAsync(request, HttpResponse.BodyHandlers.ofString())
        if (response.get().statusCode() != 200) {
